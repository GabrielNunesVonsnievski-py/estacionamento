...
class Mensalidades extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data = array(
            ...
            'sub_titulo' => 'Chegou a hora de listar as mensalidades cadastrados no banco de dados',
    ...
    public function core($mensalidade_id = NULL)
    {
        ...
        if(!$mensalidade_id){
            ...
            if($this->form_validation->run()){
                ...
                $this->core_model->insert('mensalidades', $data, array('mensalidade_id' => $mensalidade_id));
                ...
                $data = array(
                    ...
                    'icone_view' => 'fas fa-hand-holding-usd', 
                    'texto_modal' => 'Os dados estão corretos? </br></br> Depois de salva, só sera possível alterar a "Categoria" e "Situação"',
                    'styles' => array(
    ...
    public function del($mensalidade_id = NULL){
        ...
        if($this->core_model->get_by_id('mensalidades', array('mensalidade_id' => $mensalidade_id, 'mensalidade_status' => 0))){
            $this->session->set_flashdata('error', 'Mensalidade em aberto não pode ser excluída!');
            redirect($this->router->fetch_class());
        }
